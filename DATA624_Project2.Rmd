...
```{r lm_all_predictors_missing_values}
...
```{r lm_all_predictors_no_missing_values}
lm1 <- lm(PH ~ ., data=secondary_train_df)
summary(lm1)$adj.r.squared
```
**Step-wise Predictor selection**
The mode of the step-wise search is "both" (ie we are seraching for the best model adding predictors one by one, as well as removing one by one)."Better" models are quantified as having lower AIC (Akaikon Information Criterion). Missing values are included for this model.
```{r}
...
Generalized R-squared of a MARS model with missing values
```{r}
...
knn2$results
```
For the knn model that includes missing values, the best value of k is 5. The R-squared is 0.51.
For the knn model without missing values, the bests value of k is 7. The R-squared is 0.56.
